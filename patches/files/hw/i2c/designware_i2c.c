...
/*
 * halo-emu: local copy of QEMU v11.1.0 hw/i2c/designware_i2c.c, overlaid by
 * init.sh. Changes are marked "halo-emu:" and are all in the FIFO status
 * modelling; see tickets/0035-dw-i2c-rfne-status.md.
 */
...
/*
 * halo-emu: keep IC_STATUS in sync with the RX FIFO.
 *
 * Upstream only ever touches IC_STATUS.ACTIVITY and leaves the FIFO status
 * bits at their reset value (TFE|TFNF), so RFNE (RX FIFO Not Empty) reads 0
 * even with bytes sitting in rx_fifo. Zephyr's i2c_dw driver drains received
 * data with "while (test_bit_status_rfne(...) && xfr_len > 0)", so with RFNE
 * stuck low it never reads the FIFO, leaves xfr_len > 0 and reports -EIO for
 * an otherwise clean transfer. Model the FIFO status bits instead.
 */
static void dw_i2c_update_status(DesignWareI2CState *s)
{
    uint32_t status = s->regs[R_DW_IC_STATUS];

    status = FIELD_DP32(status, DW_IC_STATUS, RFNE,
                        !fifo8_is_empty(&s->rx_fifo));
    status = FIELD_DP32(status, DW_IC_STATUS, RFF,
                        fifo8_is_full(&s->rx_fifo));
    /*
     * The TX FIFO is not modelled: commands are pushed onto the I2C bus as
     * soon as they are written, so it is always empty and never full.
     */
    status = FIELD_DP32(status, DW_IC_STATUS, TFE, 1);
    status = FIELD_DP32(status, DW_IC_STATUS, TFNF, 1);

    s->regs[R_DW_IC_STATUS] = status;
}
...
static uint64_t dw_ic_data_cmd_reg_post_read(RegisterInfo *reg, uint64_t value)
{
    ...
    dw_i2c_update_status(s); /* halo-emu: RFNE/RFF follow the FIFO */
...
static void dw_i2c_reset_to_idle(DesignWareI2CState *s)
{
    ...
    dw_i2c_update_status(s); /* halo-emu */
...
static void dw_ic_data_cmd_reg_post_write(RegisterInfo *reg, uint64_t value)
{
    ...
    /*
     * halo-emu: repeated start, BEFORE the byte is transferred.
     *
     * The guest tags the *first* byte of a message with RESTART, so the
     * bus turnaround belongs ahead of that byte. Upstream ran this block
     * after the send/receive, which on a two-byte register write
     * ([reg, value]) issued the repeated START between the two bytes:
     * the target saw the value as a fresh register address and the write
     * landed in the wrong register. Reads were unaffected — their
     * restart already preceded the receive below — which is why this
     * stayed invisible until a target that checks its own registers
     * (the BMA580/QMC6308 sensor models) was attached.
     */
...
static void dw_ic_enable_reg_post_write(RegisterInfo *reg, uint64_t value)
{
    ...
    /*
     * halo-emu: only ABORT aborts the transfer. TX_CMD_BLOCK merely holds
     * commands in the TX FIFO until it is cleared; treating it as a user
     * abort (as upstream does) corrupts any transfer that uses the bit.
     */
    if (value & R_DW_IC_ENABLE_ABORT_MASK) {
...
static uint64_t dw_ic_tx_tl_reg_pre_write(RegisterInfo *reg, uint64_t value)
{
     ...
     *
     * halo-emu: the Halo devicetree sets tx_threshold = <16>, i.e. the full
     * FIFO depth, which Zephyr writes verbatim. Clamp it silently instead of
     * logging a guest error on every controller init; only report values that
     * are out of range for the register.
     */
    if (value > DESIGNWARE_I2C_TX_FIFO_SIZE - 1) {
        if (value > DESIGNWARE_I2C_TX_FIFO_SIZE) {
            qemu_log_mask(LOG_GUEST_ERROR,
